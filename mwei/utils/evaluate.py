...
class Main(object):
    def __init__(self, args):
        ...
        if (
            "test.cupt" in self.args.prediction_file.name
            or "system" in self.args.gold_file.name
        ):
            tsvlib.warn(
                "Something looks wrong in the gold & system arguments.\n"
                ...
                pred_file=self.args.prediction_file,
            )
        ...
        while self.gold or self.pred:
            ...
            if self.args.train_file:
                for whetherseen in [True, False]:
                    g = seen.mweinfo_per_whetherseen(
                        mweinfos_gold, "LEMMA", whetherseen
                    )
                    p = seen.mweinfo_per_whetherseen(
                        mweinfos_pred, "LEMMA", whetherseen
                    )
                    self.add_to_stats(
                        sent_gold,
                ...
                for variantness in [True, False]:
                    ...
                    g = seen.mweinfo_per_variantness(
                        mweinfos_gold, "LEMMA", "FORM", variantness
                    )
                    p = seen.mweinfo_per_variantness(
                        mweinfos_pred, "LEMMA", "FORM", variantness
                    )
                    self.add_to_stats(
                        sent_gold,
            ...
            if not seen.mwe_fieldindex_sets["LEMMA"]:
                ...
                for variantness in [True, False]:
                    ...
                    field_variantness2stats[
                        ...
                        prefix, baseline=field_whetherseen2stats[("LEMMA", True)]
                    )
                    field_variantness2stats[
                        ("LEMMA", "FORM", variantness)
                    ].c_mwebased.print_p_r_f(prefix)
    ...
    def add_to_stats(
        ...
        *,
        ...
        if self.args.debug:
            target_stats.c_mwebased.print_debug(
                sent_gold, inc_mwe, g_tokensets, p_tokensets, debug_header
            )
            target_stats.c_tokbased.print_debug(
                sent_gold, inc_tok, g_tokensets, p_tokensets, debug_header
            )
            print("DEBUG: +------------------------------------")
...
class SeenInfo:
    ...
    def __init__(self, train_file):
        ...
        if self.train_file is not None:
            ...
            for field_name in TRAIN_FIELD_NAMES:
                self.mwe_fieldindex_sets[field_name] = self._calc_mwe_fieldindex_sets(
                    field_name, sents
                )
    ...
    def mweinfo_per_whetherseen(
        ...
        r"""Return a sublist of MWEInfo instances that are {multi,single}-token."""
        return [
            m for m in mweinfos if whetherseen == self._seen_in_train(m, field_name)
        ]
    ...
    def _seen_in_train(self, mweinfo: tsvlib.MWEInfo, field_name: str):
        ...
        field_counter = tsvlib.FrozenCounter(
            mweinfo.sentence.words[i].get(field_name, "_")
            for i in mweinfo.token_indexes
        )
        return field_counter in self.mwe_field_sets[field_name]

    def mweinfo_per_variantness(
        self,
        mweinfos: list,
        seen_field_name: str,
        variant_field_name: str,
        variantness: bool,
    ):
        ...
        return [
            m
            for m in mweinfos
            if self._variant_of_train(
                m, seen_field_name, variant_field_name, variantness
            )
        ]

    def _variant_of_train(
        self,
        mweinfo: tsvlib.MWEInfo,
        seen_field_name: str,
        variant_field_name: str,
        variantness: bool,
    ):
...
#################################################
Increment = collections.namedtuple(
    "Increment", "plus_g plus_p plus_correct pairing"
)  # (int, int, int, dict)
...
class MatchCounter:
    ...
    def print_debug(
        ...
        print(
            "DEBUG: | {header} Mapping gold=>pred ({name}): {debug_pairing}  "
            "@@ P+={plus_correct}/{plus_p} R+={plus_correct}/{plus_g}".format(
                header=header,
                name=self.name,
                debug_pairing=pairing2t(inc.pairing),
                plus_correct=inc.plus_correct,
                plus_g=inc.plus_g,
                plus_p=inc.plus_p,
            )
        )
        ...
        for map_gold, map_pred in inc.pairing.items():
            sentence = " ".join(
                GOLDPRED_FMT[(i in map_gold, i in map_pred)].format(
                    tok.get("FORM", "_")
                )
                for i, tok in enumerate(sent.words, 1)
            )
            print(
                "DEBUG: | {header} => MATCH gold/pred ({name}): {sentence}".format(
                    header=header, name=self.name, sentence=sentence
                )
            )

        for subname, pairs in [
            ("gold", {(g, frozenset()) for g in set(g_tokensets) - inc.pairing.keys()}),
            (
                "pred",
                {
                    (frozenset(), p)
                    for p in set(p_tokensets) - set(inc.pairing.values())
                },
            ),
        ]:
            for g_ts, p_ts in pairs:
                sentence = " ".join(
                    GOLDPRED_FMT[(i in g_ts, i in p_ts)].format(tok.get("FORM", "_"))
                    for i, tok in enumerate(sent.words, 1)
                )
                print(
                    "DEBUG: | {header} => FAIL: ONLY {subname} ({name}): {sentence}".format(
                        subname=subname,
                        header=header,
                        name=self.name,
                        sentence=sentence,
    ...
    def increment_tokbased(self, g_tokensets: list, p_tokensets: list):
        ...
        inc = Increment(
            sum(len(m) for m in g_tokensets),
            sum(len(m) for m in p_tokensets),
    ...
    def print_p_r_f(self, prefix: str):
        ...
        print(
            ...
            "F={f1:.4f}".format(
                self=self, precision=precision, recall=recall, f1=f1, prefix=prefix
            )
        )
...
class Statistics:
    ...
    def __init__(self, matchcounter_kwargs: dict):
        self.c_mwebased = MatchCounter(
            "MWE-based", **matchcounter_kwargs
        )  # exact match
        self.c_tokbased = MatchCounter(
            "Tok-based", **matchcounter_kwargs
        )  # fuzzy match

    def increment_stats(
        self, g_tokensets: list, p_tokensets: list
    ) -> (Increment, Increment):
    ...
    def print_mwebased_proportion(self, prefix: str, baseline: "Statistics"):
        ...
        print(
            "* {}MWE-proportion: gold={}/{}={:.0%} pred={}/{}={:.0%}".format(
                prefix,
                n_gold_self,
                n_gold_baseline,
                n_gold_self / (n_gold_baseline or 1),
                n_pred_self,
                n_pred_baseline,
                n_pred_self / (n_pred_baseline or 1),
            )
        )
...
class ParsemeBipartiteGraph:
    def __init__(self, g_tokensets: list, p_tokensets: list):
        ...
        self.mapping = {
            g_tokensets[i_g]: p_tokensets[i_p] for (i_g, i_p) in result_index_pairs
        }
