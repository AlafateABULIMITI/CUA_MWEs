...
CONLLUP_FIELDS = (
    "ID FORM LEMMA UPOS XPOS FEATS HEAD DEPREL DEPS MISC PARSEME:MWE".split()
)
...
class TSVSentence:
    ...
    def __str__(self):
        return "TSVSentence({!r}, {!r}, {!r}, {!r})".format(
            self.filename, self.lineno_beg, self.words, self.contractions
        )
        ...
        for token_index, token in enumerate(self.words):
            ...
            for mwe_id, mwe_categ in token.mwes_id_categ():
                ...
                if token_index in mwe_info.token_indexes:
                    warn(
                        "Ignoring repeated mwe_id ({mwe_id}) in PARSEME:MWE field",
                        mwe_id=mwe_id,
                    )
                else:
    ...
    def iter_mwe_fields_and_normalizedindexes(
        self, field_name: str, fallback_field_name="FORM"
    ):
        ...
        for mweinfo in self.mwe_infos().values():
            yield mweinfo.field_and_normalizedindex_pairs(
                field_name, fallback_field_name
            )

    def iter_mwe_fields_including_span(
        self, field_name: str, fallback_field_name="FORM"
    ):
...
class MWEInfo(collections.namedtuple("MWEInfo", "sentence category token_indexes")):
    ...
    def field_and_normalizedindex_pairs(
        self, field_name: str, fallback_field_name: str
    ):
        ...
        return frozenset(
            (
                self.sentence.words[i].get_fallback(field_name, fallback_field_name),
                i - min_index,
            )
...
def warn(message, *, warntype="WARNING", position=None, **format_args):
    ...
    if _WARNED[message] <= _MAX_WARNINGS:
        ...
        for i, msg in enumerate(msg_list):
            ...
            print(
                line_beg, position, warn, ": ", msg, line_end, sep="", file=sys.stderr
            )
